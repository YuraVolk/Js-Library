...
export class BackfaceCarouselComponent extends LitElement {
	...
	protected rotateCarousel(newCurrentImage: number) {
		...
		this._carouselWrap.style.transform = `rotate${this.isHorizontal ? "Y" : "X"}(${newCurrentImage * -theta}rad)`;
	...
	setupCarousel() {
		...
		const length = items.length,
			theta = (2 * Math.PI) / length,
			size = parseFloat(getComputedStyle(items[0])[this.isHorizontal ? "width" : "height"]);
		...
		for (let i = 0; i < length; i++) {
			...
			items[i].style.transform = `rotate${this.isHorizontal ? "Y" : "X"}(${i * theta}rad)`;
		}
